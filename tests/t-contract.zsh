#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-xcode-completions — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `zsh-xcode-completions.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='zsh-xcode-completions.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/zsh-xcode-completions.plugin.zsh"
    assert $state equals 0
}
